...
export class SettingsManager {
    ...
    static async saveUseCustomPSSH(custom_pssh) {
        console.log(custom_pssh);
        await AsyncSyncStorage.setStorage({ custom_pssh: custom_pssh });
    }

    static async getUseCustomPSSH() {
        const result = await AsyncSyncStorage.getStorage(["custom_pssh"]);
        return result["custom_pssh"] ?? false;
    }

    static async savePSSHBase64(pssh_base64) {
        await AsyncSyncStorage.setStorage({ pssh_base64: pssh_base64 });
    }

    static async getPSSHBase64() {
        const result = await AsyncSyncStorage.getStorage(["pssh_base64"]);
        return result["pssh_base64"] ?? "";
    }
}
